...
def test_todo_detail_unauthenticated(todo: Todo) -> None:
    client = APIClient()
    response = client.get(f"/api/todos/{todo.todo_id}/")
    assert response.status_code == 401


@pytest.mark.django_db
def test_todo_detail_authenticated(auth_client: APIClient, todo: Todo) -> None:
    response = auth_client.get(f"/api/todos/{todo.todo_id}/")
    assert response.status_code == 200
...
def test_todo_detail_not_found(auth_client: APIClient) -> None:
    response = auth_client.get("/api/todos/00000000-0000-0000-0000-000000000000/")
